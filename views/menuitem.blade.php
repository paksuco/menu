...
<a class="{{$container->getLinkClass($level,  $showActive && $item->active)}} relative"
    ...
    <div class="flex items-center">
        ...
        <span class="bg-gray-200 ml-2 font-semibold -mt-px rounded-full text-xs py-0 w-6 text-gray-700 flex items-center text-center justify-center leading-4">{{$item->getBadge()}}</span>
    </div>
    @if($hasChildren)
